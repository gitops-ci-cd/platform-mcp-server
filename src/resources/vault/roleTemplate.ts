...
const readCallback: ResourceTemplateDefinition["readCallback"] = async (uri, variables) => {
  ...
  if (segments.length < 2) {
    throw new Error(
      `Invalid role path format: ${rolePath}. Expected format with at least one '--' separator`
    );
  }
  ...
  if (!authMethod || !roleName) {
    throw new Error(
      `Invalid role path format: ${rolePath}. Could not extract auth method and role name`
    );
  }
  ...
  try {
    ...
    if (data.token_policies) {
      data.token_policies = await Promise.all(
        data.token_policies.map((name: string) =>
          readPolicy(name)
            .then((p) => p.json())
            .then((p) => p.data)
            .catch(() => name)
...
export const vaultRoleTemplate: ResourceTemplateDefinition = {
  ...
  resourceTemplate: new ResourceTemplate("vault://roles/{rolePath}", {
    list: undefined,
    complete: {
