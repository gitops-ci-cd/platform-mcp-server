...
const inputSchema = z.object({
  category: z
    .enum(Object.keys(CDK_EXAMPLES) as [string, ...string[]])
    .describe(
      "CDK construct category (core, compute, storage, networking, security, monitoring, pipeline)"
    ),
  constructType: z
    .string()
    .describe("Type of construct (e.g. lambda, s3, vpc, iam, cloudwatch, codepipeline)"),
  name: z.string().describe("Name of the construct/resource"),
  parameters: z
    .record(z.any())
    .describe("Construct-specific parameters (e.g. runtime, memory, vpc config, policies, etc.)"),
});
